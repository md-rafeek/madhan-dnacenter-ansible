...
- name: Take backup of device(s) based on the IP address list and file types
  cisco.dnac.device_configs_backup_workflow_manager:
    dnac_host: "{{ dnac_host }}"
    dnac_username: "{{ dnac_username }}"
    dnac_password: "{{ dnac_password }}"
...
RETURN = r"""
# Case_1: creation and exportation of device configs
response_1:
...
class DeviceConfigsBackup(DnacBase):
    ...
    def validate_input(self):
        ...
        device_configs_backup_spec = {
            ...
            "file_types": {"type": "list", "elements": "str", "required": False},
        }
    ...
    def download_file(self, additional_status_url=None, file_id=None):
        """
    ...
    def unzip_data(self, file_id, file_data):
        ...
        try:
            ...
            file_data_encode = file_data.encode("latin1")
            zip_data = BytesIO(file_data_encode)
    ...
    def convert_binary_file(self, input_path, output_path, data_type):
        """
        Converts a raw binary file to a more readable format.

        Args:
            input_path (str): Path to the input binary file.
            output_path (str): Path to the output file.
            data_type (str): Data type of the binary data (e.g., 'int8', 'float32').
        """
        try:
            with open(input_path, 'rb') as infile, open(output_path, 'w') as outfile:
                while True:
                    ...
                    for byte in chunk:
                        ...
                        # Convert integer to desired data type
                        if data_type == 'int8':
                            outfile.write(str(int_value) + '\n')
                        elif data_type == 'float32':
                            # Interpret the byte as a single-precision float
                            float_value = struct.unpack('>f', bytes([byte*4]))[0]
                            outfile.write(str(float_value) + '\n')
                        elif data_type == 'hex':
                            hex_value = hex(int_value)
                            outfile.write(str(hex_value) + '\n')
                        elif data_type == 'bin':
                            bin_value = bin(int_value)
                            outfile.write(str(bin_value) + '\n')
                        else:
    ...
    def get_want(self, config):
        ...
        self.want["ip_address_list"] = ip_address_list
    ...
    def get_network_device_configuration_file_details(self, mgmt_ip_to_instance_id_map):
        ...
        file_types = self.want.get("file_types")
        if not file_types or not isinstance(file_types, list):
            file_types = valid_file_types

        normalized_file_types = []
        for ftype in file_types:
            if ftype.upper() not in valid_file_types:
                msg = "Invalid file_type: {0} given in the input, valid file_types: {1}".format(
                    file_types, valid_file_types)
                self.fail_and_exit(msg)

            normalized_file_types.append(ftype.upper())

        self.log("Validated file_type recieved '{0}'".format(normalized_file_types), "DEBUG")

        try:
            self.log("Processing device list for configuration file details.", "DEBUG")

            filter_file_ids = []
            for each_ip in self.want.get("mgmt_ip_to_instance_id_map"):
            # for ip_address, device_id in mgmt_ip_to_instance_id_map.items():
                self.log("Processing device with IP: {} and ID: {}".format(
                    each_ip, mgmt_ip_to_instance_id_map.get(each_ip)), "DEBUG")

                file_type_response = []
                file_ids_response = []
                for file_type in normalized_file_types:
                    self.log("Retrieving files of type '{0}' for device ID {1}".format(
                        file_type, mgmt_ip_to_instance_id_map.get(each_ip)), "DEBUG")

                    response = self.execute_get_request(
                        "configuration_archive",
                        "get_network_device_configuration_file_details_v1",
                        {
                            "networkDeviceId": mgmt_ip_to_instance_id_map.get(each_ip),
                            "fileType": file_type,
                            "offset": 1,
                            "limit": 1
                        }
                    ...
                    if response and response.get("response"):
                        ...
                        if file_id:
                            file_ids_response.append(file_id)
                            file_type_response.append(file_type)
                            self.log("Retrieved {0} file(s) for device ID {1} with fileType {2}".format(
                                len(file_id), mgmt_ip_to_instance_id_map.get(each_ip), file_type), "DEBUG")
                    else:
                        self.log("No config files found for device ID {0} with fileType {1}".format(
                            mgmt_ip_to_instance_id_map.get(each_ip), file_type), "DEBUG")
                        continue
                
                filter_file_ids.append({
                    "device_id": mgmt_ip_to_instance_id_map.get(each_ip),
                    "ip_address": each_ip,
                    "file_ids": file_ids_response,
                    "file_types": file_type_response
                })

            if filter_file_ids:
                self.log("Final configuration file details: {0}".format(
                    self.pprint(filter_file_ids)), "INFO")
                return filter_file_ids
            return None

        except Exception as e:
            msg = "An error occurred in 'get_network_device_configuration_file_details': {}".format(str(e))
            self.log(msg, "ERROR")
            self.fail_and_exit(msg)
    ...
    def get_configuration_file_ids(self, mgmt_ip_to_instance_id_map, file_password):
        ...
        for item in response:
            ...
            if file_ids and isinstance(file_ids, list):
                ...
                self.log("Unmasked Params: {0}".format(unmasked_params), "INFO")
                self.log(f"Added file ID '{file_ids}'", "INFO")
                id_list.append(unmasked_params)

        if not id_list:
            self.log("No valid file IDs extracted from configuration file details.", "WARNING")
        else:
            self.log(f"Extracted file IDs: {id_list}", "INFO")

        return id_list

    # def unmasked_fileid_device_configurations_params(self, file_password, id_list):
    #     """
    #     Creates parameters for exporting device configurations from Cisco Catalyst Center.
    #     Parameters:
    #         file_password (str): The password to secure the exported device configurations.
    ...
    #     """
    #     # Construct the parameters dictionary for exporting device configurations
    #     unmasked_fileid_device_configurations_params = {
    #         "id": id_list,
    #         "password": file_password
    #     }

    #     return unmasked_fileid_device_configurations_params
    
    def download_unmasked_raw_device_configuration(self, id_list, file_types, file_password):
        """
        Downloads unmasked (raw) device configuration for each file ID.
        """
        try:
            if not id_list or not file_password:
                raise ValueError("Missing 'id_list' or 'file_password' in parameters")
            ...
            for file_id in id_list:
                ...
                self.log(f"Requesting export for file ID: {file_id} with payload: {payload}", "INFO")
                ...
                self.log("Received API response for  Downloads unmasked (raw) device configuration: {}".format(
                    response.data), "DEBUG")
                # response = response.data

                if not response:
                    self.log("Empty response received for Downloads unmasked (raw) device configuration of file ID: {0}".
                             format(file_id), "WARNING")
                    continue

                zipped_file_path = "{0}/{1}_{2}.".format(self.want.get("file_path"),
                                                            file_types[file_list_index], file_id)
                try:
                    with open(zipped_file_path, "wb") as file:
                        file.write(response.data)
                    created_file_list.append(zipped_file_path)
                    self.log("Downloaded the backup to {0} without unzipping.".format(
                        zipped_file_path), "INFO")
                except OSError as e:
                    self.log("Failed to write zipped backup to {0}. Error: {1}".format(
                        zipped_file_path, str(e)), "ERROR")
                    continue
                file_list_index += 1
            ...
            self.msg = "An error occurred in 'download_unmaskedraw_device_configuration_as_z_ip_v1': {}".format(e), "ERROR"
    ...
    def get_diff_merged(self):
        ...
        if self.compare_dnac_versions(self.get_ccc_version(), "2.3.7.6") <= 0:
            ...
            # Step 1: Map device IPs to instance IDs
            mgmt_ip_to_instance_id_map = self.get_device_id_list(params)
            self.log("Management IP to Instance ID Map: {0}".format(mgmt_ip_to_instance_id_map), "DEBUG")

            # Step 2: Retrieve configuration file details (IP and file IDs)
            #ip_to_file_details = self.get_configuration_file_ids(mgmt_ip_to_instance_id_map, file_password)
            ip_to_file_details = self.get_network_device_configuration_file_details(mgmt_ip_to_instance_id_map)
            self.log("Retrieved Configuration File Details: {}".format(ip_to_file_details), "DEBUG")

            if not ip_to_file_details:
                self.log("No file details returned for processing.", "ERROR")
                return self

            # Step 3: Process and download files for each device IP
            for file_details in ip_to_file_details:
                ip_address = file_details.get("ip_address")
                file_ids = file_details.get("file_ids")

                if not file_ids:
                    self.log(f"No file IDs found for IP {ip_address}, skipping.", "ERROR")
                    continue

                # Step 4: Create a folder for the IP address if it doesn't exist
                ip_folder_path = os.path.join(file_path, ip_address.replace('.', '_'))
                if not os.path.exists(ip_folder_path):
                    os.makedirs(ip_folder_path, exist_ok=True)
                    self.log(f"Created directory for IP address {ip_address}: {ip_folder_path}", "INFO")

                # Step 5: Download each file for the current IP address
                for file_id in file_ids:
                    self.log(f"Downloading file with ID {file_id} for IP {ip_address}", "INFO")
                    file_id, downloaded_file  = self.download_file(None, file_id)
                    ...
                    download_status = self.unzip_data(file_id, downloaded_file)
                    if download_status:
                        with open(download_status, 'rb') as file:
                            my_secrets = file.read()
                            try:
                                download_status = my_secrets.decode('utf-8')
                            except UnicodeDecodeError:
                                download_status = my_secrets.decode('latin-1')
                        #my_secrets = self.convert_binary_file(download_status, download_status + ".cfg", "bin")
                        # with pyzipper.AESZipFile(download_status) as zf:
                        #     zf.setpassword(file_password)
                        #     my_secrets = zf.read(file_id)
                        self.msg = (
                            "Task has been successfully performed on {0} device(s) and skipped on {1} device(s). "
                            "The backup configuration files can be found at: {2} {3}.".format(
                                len(mgmt_ip_to_instance_id_map),
                                len(self.skipped_devices_list),
                                pathlib.Path(self.want.get("file_path")).resolve(),
                                download_status
                    ...
                    # if result:
                    #     # Ensure result is a tuple (file_id, file_data)
                    #     file_id, file_data = result

                    #     # Check if file_data is in bytes format
                    #     if isinstance(file_data, bytes):
                    #         # Process file as bytes
                    #         timestamp = datetime.datetime.now().strftime("%d_%b_%Y_%H_%M_%S_%f")[:-3]
                    #         unique_name = f"{timestamp}_{file_id}"
                    #         temp_folder_path = os.path.join(ip_folder_path, unique_name)
                    #         os.makedirs(temp_folder_path, exist_ok=True)

                    #         # Create and save the file
                    #         plain_file_path = os.path.join(temp_folder_path, f"{unique_name}")
                    #         with open(plain_file_path, "wb") as f:
                    #             f.write(file_data)

                    #         # If unzip_required is True, handle unzipping
                    #         if unzip_required:
                    #             try:
                    ...
                    #                 if extracted_files:
                    #                     extracted_file_path = os.path.join(temp_folder_path, extracted_files[0])
                    #                     os.rename(extracted_file_path, plain_file_path)
                    #             except Exception as e:
                    #                 self.log(f"Failed to unzip or process zip for file ID {file_id}: {str(e)}", "ERROR")
                    #                 continue

                    #         # Final zip creation
                    #         final_zip_path = os.path.join(ip_folder_path, f"{unique_name}.zip")
                    #         shutil.make_archive(base_name=final_zip_path.replace(".zip", ""), format='zip', root_dir=temp_folder_path)
                    #         shutil.rmtree(temp_folder_path)
                    #         self.log(f"Created final ZIP for IP {ip_address}: {final_zip_path}", "INFO")
                    #     else:
                    #         self.log(f"Expected bytes for file_data, but got {type(file_data)}", "ERROR")

            self.log(f"Backup completed. Zipped files stored at: {file_path}", "INFO")
            self.log(f"Desired State (want): {params}", "DEBUG")
...
def main():
    ...
    for config in ccc_device_configs_backup.validated_config:
        ccc_device_configs_backup.log("Processing config: {}".format(config), "INFO")
