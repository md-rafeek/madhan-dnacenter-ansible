      ...
      device_info:
        ...
        suboptions:
          ...
          is_sudi_required:
            ...
            required: false
          authorize_device:
            description: Update the Authorization flag to the device.
            type: bool
            required: false
      site_name:
...
class PnP(DnacBase):
    ...
    def authorize_devices(self, device_id=None):
        """
        Authorize the devices based on the authorize device field provided in the playbook.

        Parameters:
            self: An instance of a class used for interacting with Cisco Catalyst Center.
            device_id (str): The unique identifier of the device to be authorize or None.

        Returns:
            dict: The response returned after authorized the devices.
        """

        device_id_list = []
        if device_id:
            device_id_list.append(device_id)
            self.log("Starting to authorize the device: {0}".format(device_id), "INFO")
        else:
            ...
            for each_device in pnp_devices:
                ...
                authorize_device = each_device.get("deviceInfo", {}).get("authorize_device")

                if not authorize_device:
                    continue
                ...
                if device_response and isinstance(device_response, dict):
                    ...
                    authorize_stat = device_info.get("authStatus")

                    if not authorize_stat:
                        self.log("Adding the to list to update device authorization: '{0}'.".format(
                            device_response.get("id")), "DEBUG")
                        device_id_list.append(device_response.get("id"))
                    else:
                        self.log("Device already in authorization state: '{0}'.".format(
                            device_response.get("id")), "INFO")

        if not device_id_list:
            self.log("Device(s) are already in authorization state", "INFO")
            return None
        ...
        try:
            authorize_response = self.dnac_apply['exec'](
                ...
                params={"deviceIdList": device_id_list},
                op_modifies=True,
            )
            self.log("Response from 'authorize_device_v1' API for errored state resolution: {0}".
                     format(str(authorize_response)), "DEBUG")

            if authorize_response and isinstance(authorize_response, dict):
                self.log("Device(s) '{0}' authorized successfully.".format(
                    str(device_id_list)), "INFO")
                return authorize_response
            else:
                msg = "Failed to authorize the device(s): {0}".format(str(device_id_list))
                self.log(msg, "INFO")
                self.fail_and_exit(msg)

        except Exception as e:
            self.msg = "Failed to execute 'authorize_device_v1' for device IDs '{0}': ".format(
                device_id)
            self.log(self.msg + str(e), "ERROR")
            self.fail_and_exit(self.msg)
        ...
        if self.want.get('pnp_params'):
            ...
            for each_device in pnp_devices:
                ...
                if device_response and isinstance(device_response, dict):
                    ...
                    if not authorize_stat:
                        self.log("Updating device authorization for serial: '{0}'.".format(
                            serial_number), "DEBUG")
                        self.authorize_devices(device_response.get("id"))
                    else:
                        self.log("Device '{0}' already authorized. No update needed.".format(
                            serial_number), "DEBUG")
            ...
            if devices_not_exist and not site:
                ...
                self.bulk_devices_import(devices_not_exist)
                self.log("Updating authorization state to new device(s).", "INFO")
                self.authorize_devices()
                return self
